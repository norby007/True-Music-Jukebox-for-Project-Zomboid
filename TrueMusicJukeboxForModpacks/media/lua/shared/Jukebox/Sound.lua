...
function JukeboxSound:setVolume(volume, distance)
	...
	if self.sound3d then
		...
		local modifier = self.volume * 1.1

		-- Gives vanilla 3D sound similar max perceived loudness to our 3D sound.
		self.audibleVolume = self.volume * modifier
	else
	...
	-- Godspeed, decorators.
	self.emitter:setVolume(self.id, math.min(self.audibleVolume * mainVolume * mainVolumeBoost, 5))
