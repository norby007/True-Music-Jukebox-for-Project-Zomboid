...
JukeboxClient.update = function(packet)
	if not packet then return end

	if packet.activeLocationsLoaded then
		Jukebox.activeLocationsLoaded = true
		return
	end

	-- Set local activeLocations table from server's mod data.
	for key, jukeboxData in pairs(packet) do
		Jukebox.activeLocations[key] = jukeboxData
		Jukebox.addBatchedKey(key, jukeboxData.batch)
	end

	if SandboxVars.TrueMusicJukebox.requestJukeboxDataOnConnect and not Jukebox.activeLocationsLoaded then 
		-- Any player on this client is fine. Tells server to continue sending
		-- the rest of the jukebox data since it is not yet finished.
