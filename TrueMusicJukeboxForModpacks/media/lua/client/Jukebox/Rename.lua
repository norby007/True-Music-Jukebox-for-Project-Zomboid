...
function JukeboxRenamePanel:initialise()
    ...
    self.entry = ISTextEntryBox:new(JukeboxMenus.getMenuName(self.key, self.keyRing), self.entryX, self.entryY, self.entryWidth, JukeboxRenamePanel.fontHeightSmall + 2 * 2)
...
function JukeboxRenamePanel:onClick(button)
    ...
    if button.internal == "OK" then
        self.keyRing[self.key] = self.entry:getText()
        self:setVisible(false)
        self:removeFromUIManager()
    end
...
function JukeboxRenamePanel:new(x, y, width, height, player, key)
    ...
    o.player = player
    o.key = key
    o.keyRing = Jukebox.getPortableKeys(player)
